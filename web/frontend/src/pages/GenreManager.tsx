...
import { buildGenreDetailPath, getGenresFromResponse, normalizeGenreDetail } from "../lib/genres";
...
export function GenreManager({ nav, theme, t }: { nav: Nav; theme: Theme; t: TFunction }) {
  ...
  const { lang } = useI18n();
  const { data, refetch } = useApi<ReadonlyArray<GenreInfo> | { genres: ReadonlyArray<GenreInfo> }>("/genres");
  const [selected, setSelected] = useState<string | null>(null);
  const [formMode, setFormMode] = useState<"hidden" | "create" | "edit">("hidden");
  const [form, setForm] = useState<GenreFormData>(EMPTY_FORM);
  const genres = getGenresFromResponse(data);

  // Only show genres matching current language, plus custom project genres
  const filteredGenres = genres.filter((g) => g.language === lang || g.source === "project");
  ...
  const { data: detailData } = useApi<GenreDetail>(validSelected ? buildGenreDetailPath(validSelected, selectedGenre?.language ?? lang) : "");
  const detail = normalizeGenreDetail(detailData);
