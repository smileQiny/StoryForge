...
interface StyleAnalyzeApiResponse {
  ...
  readonly styleGuidance?: ReadonlyArray<string>;
...
function normalizeStyleProfile(
  data: StyleAnalyzeApiResponse,
  fallbackSourceName: string,
): StyleProfile {
  return {
    sourceName: data.sourceName ?? fallbackSourceName,
    avgSentenceLength: data.avgSentenceLength ?? data.stats?.averageSentenceLength ?? 0,
    sentenceLengthStdDev: data.sentenceLengthStdDev ?? 0,
    avgParagraphLength: data.avgParagraphLength ?? data.stats?.averageParagraphWords ?? 0,
    vocabularyDiversity: data.vocabularyDiversity ?? data.fingerprint?.lexicalDiversity ?? 0,
    topPatterns: [...(data.topPatterns ?? data.fingerprint?.topPatterns ?? data.styleGuidance ?? [])],
    rhetoricalFeatures: [...(data.rhetoricalFeatures ?? data.fingerprint?.rhetoricalFeatures ?? [])],
  };
}
...
export function StyleManager({ nav, theme, t }: { nav: Nav; theme: Theme; t: TFunction }) {
  ...
  const [analysisResult, setAnalysisResult] = useState<StyleAnalyzeApiResponse | null>(null);
  const [loading, setLoading] = useState(false);
  const [analyzeStatus, setAnalyzeStatus] = useState("");
  const [importBookId, setImportBookId] = useState("");
  const [importStatus, setImportStatus] = useState("");
  const { data: booksData } = useApi<BooksResponse<BookSummary>>("/books");
  const books = getBooksFromResponse(booksData);
  ...
  const handleAnalyze = async () => {
    ...
    setAnalyzeStatus("");
    try {
      const data = await fetchJson<StyleAnalyzeApiResponse>("/style/analyze", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ text, sourceName: sourceName || "sample" }),
      });
      setAnalysisResult(data);
      setProfile(normalizeStyleProfile(data, sourceName || "sample"));
    } catch (e) {
  ...
  const handleImport = async () => {
    if (!importBookId || !text.trim() || !analysisResult) return;
    setImportStatus("Importing...");
    try {
      await postApi(`/books/${importBookId}/style/import`, {
        source: sourceName || "sample",
        summary: analysisResult.styleGuide || sourceName || "sample",
        fingerprint: analysisResult.fingerprint ?? {},
        guidance: analysisResult.styleGuidance ?? [],
        notes: text,
      });
            ...
            <div className={`border ${c.cardStatic} rounded-lg p-5 space-y-4`}>
              ...
              <div className="border-t border-border pt-4 mt-4 space-y-3">
                ...
                <button
                  ...
                  disabled={!importBookId || !analysisResult}
