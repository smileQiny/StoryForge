...
import { getProviderOptions, saveProjectConfig } from "./ConfigView";
...
describe("getProviderOptions", () => {
  it("includes a claude profile choice for model routing", () => {
    const t = ((key: string) => key) as never;
    expect(getProviderOptions(t).some((option) => option.value === "claude")).toBe(true);
  });
});
