...
interface AnalyticsData {
  ...
  readonly chapterStatusCount?: Record<string, number>;
}

function normalizeAnalyticsData(data: AnalyticsData) {
  ...
  return {
    ...
    totalChapters,
    totalWords,
    avgWordsPerChapter: data.avgWordsPerChapter ?? (totalChapters > 0 ? Math.round(totalWords / totalChapters) : 0),
    statusDistribution: data.statusDistribution ?? data.chapterStatusCount ?? {},
  };
}
...
export function Analytics({ bookId, nav, theme, t }: { bookId: string; nav: Nav; theme: Theme; t: TFunction }) {
  ...
  const analytics = normalizeAnalyticsData(data);
  const statuses = Object.entries(analytics.statusDistribution);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        <StatCard label={t("analytics.totalChapters")} value={analytics.totalChapters.toString()} c={c} />
        <StatCard label={t("analytics.totalWords")} value={analytics.totalWords.toLocaleString()} c={c} />
        <StatCard label={t("analytics.avgWords")} value={analytics.avgWordsPerChapter.toLocaleString()} c={c} />
      </div>
